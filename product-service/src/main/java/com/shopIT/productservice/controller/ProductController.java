...
//    Either you can use this way using Slf4j library, or Lombok already provides @Slf4j annotation to use directly.
//    private static final Logger logger = LoggerFactory.getLogger(ProductController.class);
    ...
    //    @GetMapping(value = "/getAllProducts", params = "version=1") and  @GetMapping(value = "/getAllProducts", params = "version=2")
    //    e.g. postman call: http://localhost:8080/getAllProducts?version=1 and http://localhost:8080/getAllProducts?version=2

    // 3. (Custom) Headers versioning:
    //    @GetMapping(value = "/getAllProducts", headers = "X-API-VERSION=1") and @GetMapping(value = "/getAllProducts", headers = "X-API-VERSION=2")
    //    e.g. postman call: http://localhost:8080/getAllProducts with header key: X-API-VERSION and value: 1 and http://localhost:8080/getAllProducts with header key: X-API-VERSION and value: 2

    // 4. Media type OR Content Negotiation OR Accept Header Versioning:
    //    @GetMapping(value = "/getAllProducts", produces = "application/v1+json") and @GetMapping(value = "/getAllProducts", produces = "application/v2+json")
    //    e.g. postman call: http://localhost:8080/getAllProducts using standard header key: ACCEPT and value: application/v1+json and http://localhost:8080/getAllProducts using standard header key: ACCEPT and value: application/v2+json

    // NOTE: Every API versioning ways have some drawbacks so don't use unless necessary.
    ...
    public ResponseEntity<List<ProductDtoResponse>> getAllProductsV1() {
        ...
        if (bucket.tryConsume(1)) {
            log.info(String.format(ProductConstants.API_CHECK, bucket.getAvailableTokens() + 1, "getAllProducts allowed."));
