...
public class InventoryController {
    ...
    public ResponseEntity<List<InventoryDtoResponse>> quantityInStock(@RequestParam List<String> skuCode) {
        ...
        if (bucket.tryConsume(1)) {
            log.info(String.format(InventoryConstants.API_CHECK, bucket.getAvailableTokens() + 1, "quantityInStock allowed."));
    ...
    public ResponseEntity<String> addInInventory(@RequestBody InventoryDtoRequest inventoryDtoRequest) {
        ...
        if (bucket.tryConsume(1)) {
            log.info(String.format(InventoryConstants.API_CHECK, bucket.getAvailableTokens() + 1, "addInInventory allowed."));
